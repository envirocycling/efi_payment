    ...
    <div id="tab-1" class="tab">
		...
		<?php include('templates/per_branch.php');?>
            <article>
                ...
                @session_start();
                if (isset($_POST['submit'])) {
                    $query = "SELECT * FROM payment WHERE status='approved' and date>='" . $_POST['from'] . "' and date<='" . $_POST['to'] . "' and branch_code like '%" . $_GET['branch'] . "%'";
                } else {
                    $query = "SELECT * FROM payment WHERE status='approved' and branch_code like '%" . $_GET['branch'] . "%'";
                }
