...
if(isset($_POST['submit'])) {
    ...
    if($position == 'Reliever') {
        $query = "SELECT * FROM payment WHERE status='approved' and (branch_code like '%{$branch1}%' or branch_code like '%{$branch2}%' or branch_code like '%{$branch3}%') and signatory like '%{$initial}%' and (date >= '{$from}' and date <= '{$to}');";
    } else {
        $query = "SELECT * FROM payment WHERE status='approved' and (branch_code like '%{$branch1}%' or branch_code like '%{$branch2}%' or branch_code like '%{$branch3}%') and (date>='{$from}' and date<='$to');";
    }

} else {

    if ($position == 'Reliever') {
        $query = "SELECT * FROM payment WHERE status='approved' and (branch_code like '%{$branch1}%' or branch_code like '%{$branch2}%' or branch_code like '%{$branch3}%') and signatory like '%{$initial}%'";
    } else {
        $query = "SELECT * FROM payment WHERE status='approved' and (branch_code like '%{$branch1}%' or branch_code like '%{$branch2}%' or branch_code like '%{$branch3}%')";
    }
}
